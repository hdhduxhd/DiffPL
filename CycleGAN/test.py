...
if __name__ == '__main__':
    ...
    for i, sample in enumerate(domain_loaderT):
        ...
        _, target_new_pl, input_noise, output1_noise, t = model.get_output_B(target_prob_pl, type1='one', type2='one')
        ...
        if before_cup < after_cup:
            print('image:{%d} with timestep:{%d}, before_cup{%.4f}<after_cup{%.4f}'%(i,0,before_cup,after_cup))
            flag = True
        if before_disc < after_disc:
            print('image:{%d} with timestep:{%d}, before_disc{%.4f}<after_disc{%.4f}'%(i,0,before_disc,after_disc))
            flag = True
        if flag:
            nums.append(i)
