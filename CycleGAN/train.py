...
if __name__ == '__main__':
    ...
    for epoch in range(opt.epoch_count, opt.n_epochs + opt.n_epochs_decay + 1):    # outer loop for different epochs; we save the model by <epoch_count>, <epoch_count>+<save_latest_freq>
        ...
        for i in range(max(len(domain_loaderS),len(domain_loaderT))):
            ...
            model.set_input(data, mode=i%3)         # unpack data from dataset and apply preprocessing
            model.optimize_parameters(mode=i%3)   # calculate loss functions, get gradients, update network weights
